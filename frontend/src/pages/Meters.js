...
const Meters = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert('Error creating meter: ' + (error.response?.data?.error || error.message));
    }
  };
    ...
    <div style={styles.container}>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} style={styles.form}>
          ...
          <button type="submit" style={styles.submitBtn}>Create Meter</button>
        </form>
      )}
      ...
      {meters.length === 0 && (
        <p style={styles.empty}>No meters yet. Add one!</p>
      )}
    </div>
  );
};
